...
To locate the EKS managed add-ons: Configuration -> Add-ons
#### Add-ons
The available managed add-ons are shown in separate cards. EKS cluster shows 3 managed add-ons: coredns, kube-proxy, vpc-cni
#### Add-on management
EKS cluster admins have choice to edit, remove or add new add-ons to the cluster

## Amazon EKS upgrade process
#### API server nodes
- During an upgrade, EKS launches new API server nodes with the upgraded Kubernetes version to replace the existing ones
#### Automatic rollback
- If any of these checks fail, EKS reverts the infrastructure deployment and the cluster remains on the previous Kubernetes version
- Running apps are not affected and your cluster is not left in an unrecoverable state
- EKS regularly backs up all managed clusters and mechanisms exist to recover clusters if necessary
#### Possible minor service interruptions
- To update the cluster, EKS requires two to three free IP addresses from the subnets provided when created the cluster
#### Upgrade nodes and Kubernetes add-ons
- EKS doesn't modify any of the running applications, cluster worker nodes, add-ons, or Kubernetes add-ons when upgrade the cluster's control plane
- Will need to perform the necessary tasks to complete the cluster upgrade process
...
## Options for the update strategy
#### Rolling update
-  Respects PodDisruptionBudgets for the cluster
- The update fails if EKS is unable to gracefully drain the pods that are running on this node group because of a PodDisruptionBudget issue
#### Force update
- This option does not respect PodDisruptionBudgets. It forces node restarts
## Self-managed worker nodes
- #### Migrate to a new node group
	-  Create a new node group and migrate your pods to that group
- #### Update the existing self-managed node group
	- Update the CloudFormation stack for an existing node group to use the new AMI
